...
- Delivery stays 16:9, 1920×1080, 24 fps; eight seven-second proofs produce a **56-second reel**. Preserve support and regression checks for 30 fps.
...
- Four coherent scene kits will support eight templates. The user reviews the reel; AI performs layer preparation and technical QA.
...
CI-01 and CI-05 reverse direction for different editorial purposes: entry versus context reveal. CI-02 sustains travel; CI-03 completes an occlusion event and holds. CI-07 changes the camera path, not the geometry of the subject. CI-08 must preserve the subject size while changing its surroundings or it fails its named purpose.
...
| **B — The courtyard** | Near masonry at an edge, a stationary anonymous figure, receding walls and passage.        | CI-02, CI-07   | Master, solid foreground cutout, figure with grounded shadow, middle architecture, complete far plate. |
...
2. **Three primary proofs:** CI-01 Threshold Push, CI-02 Lateral Track, CI-03 Foreground Reveal with original kits A and B. Inspect full-size motion, occlusion boundaries, and linework before expanding the family.
...
6. **One review:** a 56-second silent reel, eight clean full-frame shots, optional individual replay and external template labels. Add a small first/middle/last reference strip outside the video. Keep extra technical variations in the QA report.

The first three prototypes are the next implementation step. This planning task does not generate or render the new source art.
...
- The eight clips show distinct framing/motion purposes. Variation cannot be achieved solely through different speeds or reversed travel.

### Technical checks

- Seven seconds yields exactly 168 frames at 24 fps and 210 at 30 fps. Verify the joined 56-second reel and decode it completely.
