...
export function passageGallery(
  ...
  const rows = beats
    ...
    <p class="qualification"><strong>${escape(words(beat.evidence?.kind ?? "general"))}.</strong> ${escape(beat.evidence?.qualification ?? "No evidence qualification declared.")}</p>
